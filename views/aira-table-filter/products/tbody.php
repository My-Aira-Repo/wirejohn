<?php foreach ($product_ids as $id):
    ...
    $pa_app = $attributes['pa_application'];
    $attr_count = count($pa_app->get_terms());   
?>
    <?php foreach(range(0,$attr_count -1) as $index): ?>
    <tr>
        <?php foreach ($attr_slugs as $slug): ?> 
            <?php if (isset($attributes['pa_' . $slug])): ?>
                <td>
                    <?php $pa = $attributes['pa_' . $slug]; ?>
                    <?php if ($slug == 'application'): ?>
                        <?php echo($pa->get_terms()[$index]->name); ?>
                    <?php else: ?>
